...
## 7. Routing (Wayfinder)
- **Wayfinder Enforcement**: ALWAYS implement Wayfinder for frontend routing instead of hardcoding route URLs or using Ziggy.
- Replace manual route strings (e.g., `href="/dashboard"`) with Wayfinder typed routes (e.g., `import { dashboard } from '@/routes'; href={dashboard.url(slug)}`).
- Ensure type safety between the backend and the frontend by importing the exact route definitions generated by `@laravel/vite-plugin-wayfinder` from `@/routes` or `@/actions`.
